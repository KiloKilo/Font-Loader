...
var _fileType = __webpack_require__(40);

var _fileType2 = _interopRequireDefault(_fileType);

var _woff2FeatureTest = __webpack_require__(41);

var _woff2FeatureTest2 = _interopRequireDefault(_woff2FeatureTest);
...
function parseStyleTags(version) {
    ...
    var styleSheets = [].concat(_toConsumableArray(docStyleSheets));
    var rules = [];
    ...
    rules = (_ref = []).concat.apply(_ref, _toConsumableArray(rules.map(function (rule) {
        return parseRules(rule);
    })));
    rules = rules.map(function (rule) {
        return setPreferedFont(rule);
    });
    rules = rules.filter(function (rule) {
        return rule;
    });
...
function parseRules(rule) {
    ...
    var name = nameRegex.exec(rule.cssText);
    ...
    return { name: name[1], fonts: fonts };
}

function setPreferedFont(rule) {
    var woff2 = rule.fonts.find(function (font) {
    ...
    var woff = rule.fonts.find(function (font) {
    ...
    rule.fonts.forEach(function (font) {
...
function setFonts(savedFonts) {
    savedFonts.forEach(function (font) {
        return readFont(font);
    });
}
